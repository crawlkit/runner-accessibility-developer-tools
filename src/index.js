...
class A11yDeveloperToolsRunner {
  ...
  getRunnable() {
    ...
    return function axsRunner() {
      ...
      /**
       * @param line {!String}  The error line, something like:
       *                        "
       *                        Warning: ERROR_CODE (Description) failed on the following element:
       *                        html
      ...
      function parseError(line) {
        ...
        var matches = lines.shift().match(
            /^.+?: ([A-Z0-9_]+) \((.*)\) failed on the following element.+$/
        );
        var urlMatches = lines.pop().match(
